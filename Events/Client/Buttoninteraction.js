    ...
    async execute(client, interaction) {
            ...
            if (button.permissions.voteRequired) {
                ...
                if (!voted && !client.owners.includes(interaction.member.user.id)) {
                    return interaction.reply({
                        embeds: [client.embed().setColor(client.color).setDescription(`You Need To [Vote]() For Me To Use This Command!`),],
        ...
        if (button.player) {
            if (button.player.voice) {
                if (!interaction.member.voice.channel) return await interaction.reply({ content: `> <:11:1366012222220009586> **You must be connected to a voice channel to use this \`${button.name}\` button.**`, ephemeral: true });
